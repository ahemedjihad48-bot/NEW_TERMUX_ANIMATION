...
# Keep layout safe on very small terminals.
...
ensure_dependencies() {
    # Termux-only bootstrap: install everything this project needs before
    # any menu/update logic is executed. Existing packages are left alone.
    if [ -z "${PREFIX:-}" ] || [ ! -d "${PREFIX:-}" ] || [ ! -x "${PREFIX:-}/bin/pkg" ]; then
        echo -e "${R}[!] This script must be run inside Termux.${RS}"
        exit 1
    fi

    echo -e "${Y}[*] Preparing Termux package manager...${RS}"
    if ! command -v pkg >/dev/null 2>&1; then
        echo -e "${R}[!] Termux package manager (pkg) is unavailable.${RS}"
        exit 1
    fi

    # Update package indexes quietly. If the device is offline, continue and
    # let the install step report the actual problem.
    pkg update -y >/dev/null 2>&1 || true

    # Package names are checked by their executable, so rerunning the script
    # does not reinstall packages that are already present.
    local packages=(
        zsh fish git figlet toilet ruby wget curl bat eza unzip xz-utils
        ca-certificates tar coreutils ncurses-utils
    )
    local missing=()
    local pkg_name cmd_name

    for pkg_name in "${packages[@]}"; do
        cmd_name="$pkg_name"
        case "$pkg_name" in
            xz-utils) cmd_name="xz" ;;
            coreutils) cmd_name="timeout" ;;
            ncurses-utils) cmd_name="tput" ;;
        esac
    ...
    if [ ${#missing[@]} -gt 0 ]; then
        echo -e "${Y}[*] Installing missing packages: ${missing[*]}${RS}"
        if ! pkg install -y "${missing[@]}"; then
            echo -e "${R}[!] Some required packages could not be installed.${RS}"
            echo -e "${Y}    Check your internet connection and run the script again.${RS}"
            return 1
        fi
    fi

    # lolcat is distributed as a Ruby gem and is optional for the core tool,
    # but install it automatically because the UI can use it.
    if command -v gem >/dev/null 2>&1 && ! command -v lolcat >/dev/null 2>&1; then
        echo -e "${Y}[*] Installing optional Ruby dependency: lolcat${RS}"
        gem install lolcat --no-document >/dev/null 2>&1 || true
    fi

    echo -e "${G}[√] All required dependencies are ready.${RS}"
    return 0
}
